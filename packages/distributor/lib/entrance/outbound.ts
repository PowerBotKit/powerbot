import { Activity, Attachment, BotFrameworkAdapter, CardFactory, MessageFactory } from 'botbuilder';
...
export class OutBound {
	public static async listen(adapter: BotFrameworkAdapter, cache: ICache, mq: IMQ) {
		mq.onSubscribed(channel => {
			logger.info('🚗 Subscribed to outbound broker');
		});

		mq.onMessage(async (channel, message) => {
			logger.info(
				'Subscriber received message in channel: ' + channel + ' value: ' + message
			);
			const dialog: GDUserSession = await cache.get(message);
			await adapter.continueConversation(dialog.botConversion, async turnContext => {
				if (dialog.output.type === MessageType.card) {
					const cardFile = JSON.parse(
						fs.readFileSync(path.join(__dirname, '../cards/' + dialog.output.value + '.json'))
							.toString()
					);
